...
var ws = null
const userToken = window.localStorage.getItem('user') ? JSON.parse(window.localStorage.getItem('user')).userToken : null
...
export default {
  ...
  methods: {
    ...
    startGame () {
      this.$axios.get('/room/start?roomID=' + this.roomID, {headers: {'Authorization': token}}).then(success => {
    ...
    selectQuestion () {
      ...
      this.$prompt('请选择（1-6）', '提示', {
        ...
        this.$axios.post('/room/play?roomID=' + this.roomID, {
          'type': '1',
          'data': {
            'sender': window.localStorage.getItem('user') ? JSON.parse(window.localStorage.getItem('user')).userName : null,
    ...
    createWebsocket () {
      ...
      console.log(url + this.roomID + '/' + token)
      ws = new WebSocket(url + this.roomID + '/' + token)
      // 与服务端建立连接触发
      ws.onopen = function () {
